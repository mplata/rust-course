...
use self::models::{Post, NewPostHanlder};
...
async fn get_post(
    ...
    pool: web::Data<DbPool>, blog_slug: web::Path<String>
) -> impl Responder {
    let conn = pool.get().expect("Problemas al traer la base de datos");
...
#[post("/new_post")]
async fn new_post(pool: web::Data<DbPool>, item: web::Json<NewPostHanlder>) -> impl Responder {
    let conn = pool.get().expect("Problemas al traer la base de datos");
